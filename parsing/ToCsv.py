...
def get_records(html):
    ...
    for tr in trs:
        tds = tr.find_all('td')
        record = [int(tds[0].text), tds[1].text, int(tds[4].text)]
        records.append( record )
    
    #records = sorted(records, key=itemgetter(2), reverse=True)
...
def to_csv(filename, records, sep=','):
    ...
    f.write(f'id{sep}team{sep}draw{sep}\n')
    for rec in records:
...
to_csv('./parsing/results.csv', records, ';')